stdca <- function(data, outcome, ttoutcome, timepoint, predictors, xstart=0.01, xstop=0.99, xby=0.01, 
  ...
  if (graph==TRUE) {
    ...
    if(intervention==TRUE) {
      ...
      plot(x=nb$threshold, y=nb$all, type="n" ,col="blue", xlim=c(xstart, xstop), ylim=c(ymin, ymax), xlab="Threshold probability", ylab=paste("Net reduction in interventions per",interventionper,"patients"))
      box(lwd=3)
      #PLOTTING INTERVENTIONS AVOIDED FOR EACH PREDICTOR
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          lines(interv$threshold,data.matrix(interv[paste(predictors[m],"_sm",sep="")]),col="red",lty=2)     
        } else {
          lines(interv$threshold,data.matrix(interv[predictors[m]]),col="red",lty=2)
        }
    ...
    # PLOTTING NET BENEFIT IF REQUESTED
      ...
      plot(x=nb$threshold, y=nb$all, type="l", col="blue", lwd=3 ,xlim=c(xstart, xstop), ylim=c(ymin, ymax), xlab="Threshold probability", ylab="Net benefit")
      box(lwd=3)
	  # adding treat none option
      lines(x=nb$threshold, y=nb$none,lwd=3,col="green")
      #PLOTTING net benefit FOR EACH PREDICTOR
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          lines(nb$threshold,data.matrix(nb[paste(predictors[m],"_sm",sep="")]),col="red",lwd=3) 
        } else {
          lines(nb$threshold,data.matrix(nb[predictors[m]]),col="red",lwd=3)
        }
        # adding each model to the legend
        legendlabel <- c(legendlabel, predictors[m])
        legendcolor <- c(legendcolor, m)
    ...
    # then add the legend
    legend("topright", legendlabel, cex=0.8, col=c("green","blue","red"), lwd=3, lty=2)
